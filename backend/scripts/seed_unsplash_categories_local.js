...
const downloadImage = async (url, destPath) => {
    try {
        ...
        console.error(`Failed download from ${url}:`, error.message);
        return false;
    }
};
...
const seed = async () => {
    try {
        ...
        for (const item of data) {
            ...
            const parentDest = path.join(catDir, `${item.slug}.jpg`);
            const parentSuccess = await downloadImage(item.image, parentDest);
            const parentImageDbPath = parentSuccess ? `uploads/categories/${item.slug}.jpg` : item.image;
            ...
            if (!parentCat) {
                ...
                parentCat.image = parentImageDbPath;
            ...
            for (const sub of item.subcategories) {
                ...
                const subDest = path.join(catDir, `${sub.slug}.jpg`);
                const subSuccess = await downloadImage(sub.image, subDest);
                const subImageDbPath = subSuccess ? `uploads/categories/${sub.slug}.jpg` : sub.image;
        ...
        console.log('Seeding and local image download finished successfully!');
        process.exit(0);
    } catch (err) {
